...
const LoginPage: FC = () => {
    ...
    const login = () => {
        ...
        if (!window.recaptchaVerifier) {
            window.recaptchaVerifier = new firebase.auth.RecaptchaVerifier('capthca-verifier', 
                {
                    'size': 'invisible'
                });
        }
    ...
    const confirm = () => {
        if (confirmationResult) {
            confirmationResult.confirm(password)
                .then(result => {
    ...
    return (
        <div className="login-page">
            <h1>Login</h1>
            <TextInput label="Phone" value={phone} id='phone' onChange={onChangeHandler}/>

            {codeIsRequired && <TextInput label="password" value={password} id='password' type='password'  onChange={onChangeHandler}/>}
            {loading && <Spinner />}
            {!loading && !codeIsRequired && <button className="btn" onClick={login}>Login</button>}
            {!loading && codeIsRequired && <button className="btn" onClick={confirm}>Confirm</button>}
            <SimpleAlert visible={!!message} onClose={() => setMessage('')} alertClass={alertClass}><div>{message}</div></SimpleAlert>
